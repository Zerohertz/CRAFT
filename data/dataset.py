...
class CustomDataset(CraftBaseDataset):
    def __init__(
        ...
        if '.ipynb_checkpoints' in self.img_names:
            self.img_names.pop(self.img_names.index('.ipynb_checkpoints'))
    ...
    def load_img_gt_box(self, img_gt_box_path):
        ...
        words = []
        for line in lines:
            box_info = line.strip().encode("utf-8-sig").decode("utf-8-sig").split(",")
    ...
    def load_img_gt_box_json(self, img_gt_box_path):
        word_bboxes = []
        words = []
        #print(img_gt_box_path)
        lines=json.load(codecs.open(img_gt_box_path, 'r', 'utf-8-sig'))['shapes']
        for line in lines:
            box_points = line['points']
            box_points = np.array(box_points, np.float32).reshape(4, 2)
            if len(box_points)<1:
                continue
#             box_points = order_points(box_points)
#             box_points=recoord(box_points)
            all_zero=np.all((box_points == 0))
            if all_zero:
                print('all detected boxes element are zero')
                continue
            word = line['label'].strip()
            word = word.replace('[CBOXT]','').replace('[CBOXF]','')
            if len(word)<1:
                continue
            word = word.replace('[ROT0]','').replace('[ROT90]','').replace('[ROT180]','').replace('[ROT270]','').replace('[ROT360]','')
            word = word.replace('[UNK]','U').replace('[UNK1]','U').replace('[UNK2]','UU').replace('[UNK3]','UUU').replace('[UNK4]','UUUU').replace('[UNK5]','UUUUU').replace('[UNK6]','UUUUUU')
            if word in self.do_not_care_label:
                words.append(self.do_not_care_label[0])
                word_bboxes.append(box_points)
                continue
    ...
    def load_data(self, index):
        ...
        img_gt_box_path = os.path.join(
            self.img_gt_box_dir, "%s.json" % os.path.splitext(img_name)[0]
        )
        word_bboxes, words = self.load_img_gt_box_json(
            img_gt_box_path
