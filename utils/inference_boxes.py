...
def load_japanese_gt(dataFolder, isTraing=False):
    if isTraing:
        img_folderName = "images"
        gt_folderName = "json"
    else:
        img_folderName = "images"
        gt_folderName = "json"
    ...
    gt_folder_path = os.listdir(os.path.join(dataFolder, gt_folderName))
    img_folder_path = os.listdir(os.path.join(dataFolder, img_folderName))
    if '.ipynb_checkpoints' in img_folder_path:
            img_folder_path.pop(img_folder_path.index('.ipynb_checkpoints'))
    total_imgs_bboxes = []
    total_img_path = []
    for img_path in img_folder_path:
        img_path = os.path.join(os.path.join(dataFolder, img_folderName), img_path)
#         gt_path = os.path.join(os.path.join(dataFolder, gt_folderName), img_path)
        gt_path = (
            img_path.replace(img_folderName, gt_folderName)[:-4] + '.json'
        )
        lines=json.load(codecs.open(gt_path, 'r', 'utf-8-sig'))['shapes']
        single_img_bboxes = []
        for line in lines:
            box_info_dict = {"points": None, "text": None, "ignore": None}
